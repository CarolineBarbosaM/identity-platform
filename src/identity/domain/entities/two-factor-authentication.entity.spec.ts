...
describe('TwoFactorAuthentication', () => {
  it('should create a 2FA configuration', () => {
    ...
    expect(twoFactor.getId()).toBe(
      '2fa-id',
    );

    expect(twoFactor.getUserId()).toBe(
      'user-id',
    );

    expect(twoFactor.getSecret()).toBe(
      'totp-secret',
    );
    ...
    expect(twoFactor.getCreatedAt()).toEqual(
      now,
    );
  });
  ...
  it('should enable the 2FA configuration', () => {
    ...
    expect(twoFactor.getEnabledAt()).toEqual(
      now,
    );
  });
