...
describe('EmailVerificationToken', () => {
  it('should create an unused token', () => {
    ...
    const expiresAt = new Date('2026-08-12T11:00:00.000Z');
  ...
  it('should mark the token as used', () => {
    ...
    expect(token.getUsedAt()).toEqual(new Date('2026-08-12T10:30:00.000Z'));
    expect(token.getUpdatedAt()).toEqual(new Date('2026-08-12T10:30:00.000Z'));
  });
  ...
  it('should not change the used date when already used', () => {
    ...
    clock.setNow(new Date('2026-08-12T10:45:00.000Z'));
  ...
  it('should rehydrate a token', () => {
    ...
    expect(token.getTokenHash()).toBe('hashed-token');
    expect(token.getUsedAt()).toBeNull();
    expect(token.getCreatedAt()).toEqual(new Date('2026-08-12T09:00:00.000Z'));
    expect(token.getUpdatedAt()).toEqual(new Date('2026-08-12T09:30:00.000Z'));
  });
});
