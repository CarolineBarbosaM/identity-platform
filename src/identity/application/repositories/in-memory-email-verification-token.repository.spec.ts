...
describe('InMemoryEmailVerificationTokenRepository', () => {
  it('should save and find a token by id', async () => {
    const repository = new InMemoryEmailVerificationTokenRepository();
  ...
  it('should return null when token does not exist by id', async () => {
    ...
    const result = await repository.findById('unknown-token');
  ...
  it('should replace an existing token with the same id', async () => {
    ...
    const firstToken = EmailVerificationToken.create(
      {
        id: 'token-id',
        userId: 'user-id',
    ...
    const secondToken = EmailVerificationToken.create(
      {
        id: 'token-id',
        userId: 'user-id',
