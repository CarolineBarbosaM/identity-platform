...
export class InMemoryPasswordResetTokenRepository implements PasswordResetTokenRepository {
  ...
  async findByUserId(userId: string): Promise<PasswordResetToken | null> {
    return this.tokens.find((token) => token.getUserId() === userId) ?? null;
  }

  async findByTokenHash(tokenHash: string): Promise<PasswordResetToken | null> {
    return (
      this.tokens.find((token) => token.getTokenHash() === tokenHash) ?? null
    );
  }

  async save(token: PasswordResetToken): Promise<void> {
    const existingIndex = this.tokens.findIndex(
      (existingToken) => existingToken.getId() === token.getId(),
    );
