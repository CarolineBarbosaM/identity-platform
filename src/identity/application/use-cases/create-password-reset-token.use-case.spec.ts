...
describe('CreatePasswordResetTokenUseCase', () => {
  it('should create and persist a password reset token', async () => {
    ...
    const tokenGenerator = {
      generate: jest.fn().mockResolvedValue('plain-token'),
    };

    const clock = new FakeClock(new Date('2026-08-12T10:00:00.000Z'));

    const useCase = new CreatePasswordResetTokenUseCase(
      repository,
      tokenHasher,
      tokenGenerator,
      clock,
    ...
    const savedToken = await repository.findByTokenHash('hashed-token');

    expect(savedToken).not.toBeNull();

    expect(savedToken?.getUserId()).toBe('user-id');

    expect(savedToken?.getTokenHash()).toBe('hashed-token');

    expect(savedToken?.getExpiresAt()).toEqual(
  ...
  it('should create a new token on every execution', async () => {
    ...
    const tokenHasher = {
      hash: jest
        .fn()
        .mockImplementation(async (token: string) => `hash-${token}`),
    ...
    expect(tokenHasher.hash).toHaveBeenCalledTimes(2);

    expect(tokenHasher.hash).toHaveBeenNthCalledWith(1, 'first-token');

    expect(tokenHasher.hash).toHaveBeenNthCalledWith(2, 'second-token');
  });
});
