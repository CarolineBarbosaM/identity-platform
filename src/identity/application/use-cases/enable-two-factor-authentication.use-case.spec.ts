import { EnableTwoFactorAuthenticationUseCase } from './enable-two-factor-authentication.use-case';
...
describe('EnableTwoFactorAuthenticationUseCase', () => {
  it('should enable 2FA with a valid code', async () => {
    ...
    const useCase = new EnableTwoFactorAuthenticationUseCase(
      repository,
      authenticator,
    );
  ...
  it('should reject when the 2FA configuration does not exist', async () => {
    ...
    ).rejects.toThrow('Two-factor authentication configuration not found');
