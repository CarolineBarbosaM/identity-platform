...
describe('PostgresEmailVerificationTokenRepository', () => {
  const createRepository = () => {
    ...
    return {
      ...
      clock,
      postgresRepository:
        new PostgresEmailVerificationTokenRepository(
          repository,
          clock,
        ),
    };
  ...
  it('should save and find a token by id', async () => {
    ...
    const result =
      await postgresRepository.findById('token-id');
  ...
  it('should return null when token does not exist', async () => {
    ...
    const result =
      await postgresRepository.findById(
        'unknown-token',
      );
  ...
  it('should persist a used token', async () => {
    ...
    expect(result?.getUsedAt()).toEqual(
      new Date('2026-08-12T10:30:00.000Z'),
    );
  });
});
