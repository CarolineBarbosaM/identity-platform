...
export class GoogleSsoProvider implements SsoProvider {
  ...
  constructor(private readonly config: GoogleSsoProviderConfig) {}
  ...
  async authenticate(code: string, state: string): Promise<SsoUserProfile> {
    ...
    if (!state) {
      throw new Error('Google authorization state is required');
    }
  ...
  private async exchangeCodeForToken(
    ...
    if (!data.access_token) {
      throw new Error('Google token response does not contain an access token');
    }
