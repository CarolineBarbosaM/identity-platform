...
describe('IdentityController', () => {
  ...
  it('should save the SSO state before creating the authorization URL', async () => {
    ...
    const result =
      await controller.ssoAuthorization({
        params: {
          provider: 'google',
        },
      });
    ...
    expect(
      ssoProvider.createAuthorizationUrl,
    ).toHaveBeenCalledTimes(1);
  ...
  it('should authenticate SSO after consuming a valid state', async () => {
    ...
    expect(
      ssoStateStore.consume,
    ).toHaveBeenCalledWith('state-123');

    expect(
      authenticateSso.execute,
    ).toHaveBeenCalledWith({
